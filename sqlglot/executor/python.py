...
def _rename(self, e):
    try:
        ...
        raise Exception(f"Could not rename {e!r}") from ex
    ...
    class Generator(generator.Generator):
        TRANSFORMS = {
            ...
            **{klass: _rename for klass in exp.ALL_FUNCTIONS},
            ...
            exp.Cast: lambda self, e: (
                f"CAST({self.sql(e.this)}, exp.DataType.Type.{e.args['to']})"
            ),
            ...
            exp.Interval: lambda self, e: (
                f"INTERVAL({self.sql(e.this)}, '{self.sql(e.unit)}')"
            ),
            ...
            exp.JSONPath: lambda self, e: (
                f"[{','.join(self.sql(p) for p in e.expressions[1:])}]"
            ),
