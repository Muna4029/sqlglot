...
DATETRUNC_BINARY_COMPARISONS: t.Dict[
    ...
    exp.GT: lambda l, dt, u, d, t: (
        l >= date_literal(date_floor(dt, u, d) + interval(u), t)
    ),
...
def cast_value(value: t.Any, to: exp.DataType) -> t.Union[None, datetime.date]:
    if not value:
        return None
...
def extract_date(cast: exp.Expression) -> t.Union[None, datetime.date]:
