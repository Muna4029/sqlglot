...
class Generator(metaclass=_Generator):
    ...
    TRANSFORMS: t.Dict[t.Type[exp.Expression], t.Callable[..., str]] = {
        **JSON_PATH_PART_TRANSFORMS,
        ...
        exp.CharacterSetColumnConstraint: lambda self, e: (
            f"CHARACTER SET {self.sql(e, 'this')}"
        ),
        ...
        exp.EnviromentProperty: lambda self, e: (
            f"ENVIRONMENT ({self.expressions(e, flat=True)})"
        ),
        exp.EphemeralColumnConstraint: lambda self, e: (
            f"EPHEMERAL{(' ' + self.sql(e, 'this')) if e.this else ''}"
        ),
        exp.ExcludeColumnConstraint: lambda self, e: (
            f"EXCLUDE {self.sql(e, 'this').lstrip()}"
        ),
        ...
        exp.InheritsProperty: lambda self, e: (
            f"INHERITS ({self.expressions(e, flat=True)})"
        ),
        exp.InlineLengthColumnConstraint: lambda self, e: (
            f"INLINE LENGTH {self.sql(e, 'this')}"
        ),
        exp.InputModelProperty: lambda self, e: f"INPUT{self.sql(e, 'this')}",
        exp.Intersect: lambda self, e: self.set_operations(e),
        exp.IntervalSpan: lambda self, e: (
            f"{self.sql(e, 'this')} TO {self.sql(e, 'expression')}"
        ),
        ...
        exp.PartitionedByBucket: lambda self, e: self.func(
            "BUCKET", e.this, e.expression
        ),
        exp.PartitionByTruncate: lambda self, e: self.func(
            "TRUNCATE", e.this, e.expression
        ),
        ...
        exp.SettingsProperty: lambda self, e: (
            f"SETTINGS{self.seg('')}{(self.expressions(e))}"
        ),
        exp.SharingProperty: lambda self, e: f"SHARING={self.sql(e, 'this')}",
        exp.SqlReadWriteProperty: lambda _, e: e.name,
        exp.SqlSecurityProperty: lambda _, e: (
            f"SQL SECURITY {'DEFINER' if e.args.get('definer') else 'INVOKER'}"
        ),
        ...
        exp.UsingTemplateProperty: lambda self, e: (
            f"USING TEMPLATE {self.sql(e, 'this')}"
        ),
        ...
        exp.WithJournalTableProperty: lambda self, e: (
            f"WITH JOURNAL TABLE={self.sql(e, 'this')}"
        ),
        exp.WithProcedureOptions: lambda self, e: (
            f"WITH {self.expressions(e, flat=True)}"
        ),
        exp.WithSchemaBindingProperty: lambda self, e: (
            f"WITH SCHEMA {self.sql(e, 'this')}"
        ),
        exp.WithOperator: lambda self, e: (
            f"{self.sql(e, 'this')} WITH {self.sql(e, 'op')}"
        ),
    ...
    RESPECT_IGNORE_NULLS_UNSUPPORTED_EXPRESSIONS: t.Tuple[
    ...
    def between_sql(self, expression: exp.Between) -> str:
        ...
        if symmetric and not self.SUPPORTS_BETWEEN_FLAGS:
            return (
                f"({this} BETWEEN {low} AND {high} OR {this} BETWEEN {high} AND {low})"
            )
    ...
    def add_column_sql(self, expression: exp.Expression) -> str:
        ...
        if isinstance(expression, exp.Schema):
            ...
            isinstance(expression, exp.ColumnDef)
            and self.ALTER_TABLE_INCLUDE_COLUMN_KEYWORD
        ):
            column_text = " COLUMN"
        else:
            column_text = ""
    ...
    def get_put_sql(self, expression: exp.Put | exp.Get) -> str:
        ...
        props_sql = (
            self.properties(props, prefix=" ", sep=" ", wrapped=False) if props else ""
        )
        this = self.sql(expression, "this")
        target = self.sql(expression, "target")
        ...
        for search, result in zip(expressions[::2], expressions[1::2]):
            if isinstance(search, exp.Literal):
                ...
                cond = exp.or_(
                    ...
                    exp.and_(
                        expression.is_(exp.Null()), search.is_(exp.Null()), copy=False
                    ),
                    copy=False,
                )
    ...
    def getextract_sql(self, expression: exp.GetExtract) -> str:
        ...
        return self.sql(
            exp.JSONExtract(this=this, expression=self.dialect.to_json_path(expr))
        )
