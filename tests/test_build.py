...
class TestBuild(unittest.TestCase):
    def test_build(self):
        ...
        for expression, sql, *dialect in [
            ...
            (
                lambda: (
                    select("x").from_("tbl").where("x > 0").where("x < 9", append=False)
                ),
            ...
            (
                lambda: (
                    ...
                    .from_("tbl")
                    .join(
                        exp.Table(this="tbl2"), join_type="left outer", join_alias="foo"
                    )
                ),
            ...
            (
                lambda: (
                    select("x").from_("tbl").with_("tbl", as_=select("x").from_("tbl2"))
                ),
                "WITH tbl AS (SELECT x FROM tbl2) SELECT x FROM tbl",
            ),
            (
            ...
            (
                lambda: (
                    select("x").with_("tbl", as_=select("x").from_("tbl2")).from_("tbl")
                ),
                "WITH tbl AS (SELECT x FROM tbl2) SELECT x FROM tbl",
            ),
            (
            ...
            (
                lambda: (
                    parse_one("SELECT a FROM tbl").assert_is(exp.Select).select("b")
                ),
                "SELECT a, b FROM tbl",
            ),
            (
            ...
            (
                lambda: (
                    exp.subquery("select x from tbl", "foo").select("x").where("x > 0")
                ),
            ...
            (
                lambda: (
                    exp.delete("tbl").where("x = 1").returning("*", dialect="postgres")
                ),
            ...
            (
                lambda: (
                    select("x")
                    .with_(
                        "var1", as_=select("x").from_("tbl2").subquery(), scalar=True
                    )
                    .from_("tbl")
                    .where("x > var1")
                ),
